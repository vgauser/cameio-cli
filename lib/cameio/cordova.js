...
CameioTask.prototype.runCordova = function(cmdName) {
  var self = this;
  ...
  if(cmdName == 'platform') {
    // `cameio platform <PLATFORM>` used to actually run `cameio platform add <PLATFORM>`
...
CameioTask.prototype.setupLiveReload = function() {
  var d = Q.defer();
...
CameioTask.prototype.addHooks = function() {
  // Add hooks which this Cameio project doesn't already have
  ...
  // loop through all the hook directories added to the cameio-cli
  ...
  function addCliHookDirectory(cliHookPath, hookDirectoryName) {
    fs.readdir(cliHookPath, function(err, files){
      // loop through each of the scripts in the cameio-cli hook directory
      if(err) return;
      for(var x=0; x<files.length; x++) {
        var hookFilename = files[x];
        if(hookFilename.indexOf('.js') === -1) return;

        // check if this hook script has already been added to this cameio project
