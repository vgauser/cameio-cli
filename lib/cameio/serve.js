...
CameioTask.prototype.run = function(cameio) {
  ...
  this.loadSettings(function(){
    ...
    if(cameio.hasFailed) return;

    cameio.latestVersion.promise.then(function(){
      cameio.printVersionWarning();
    });
  });
};


CameioTask.prototype.loadSettings = function(cb) {
  ...
  this.contentSrc = path.join('www', this.cameio.getContentSrc());
  ...
  process.stdin.on('readable', function() {
    ...
    if(input == 'restart' || input == 'r') {
      ...
      console.log('\nInvalid cameio server command'.error.bold);
      self.printCommandTips();
    }
...
CameioTask.prototype.printCommandTips = function(cameio) {
  console.log('Cameio server commands, enter:'.green.bold);
...
CameioTask.prototype.start = function(cameio) {
  try {
    var self = this;
    var app = connect();

    if (!fs.existsSync( path.resolve('www') )) {
      return cameio.fail('"www" directory cannot be found. Please make sure the working directory is an Cameio project.');
    }
    ...
    if(this.runLivereload) {
      ...
      lrServer.listen(this.liveReloadPort, function(err) {
        if(err) {
          return cameio.fail('Unable to start live reload server:', err);
        } else {
    ...
    var server = http.createServer(function(req, res){
      ...
      if(self.printConsoleLogs && req.url === '/__cameio-cli/console') {
        self.consoleLog(req);
        res.end('');
        return;
      }
...
CameioTask.prototype.serverLog = function(req, msg) {
...
CameioTask.prototype.consoleLog = function(req) {
  var body = '';
...
function injectConsoleLogScript(html) {
  try{
    ...
    <script>\n\
      // Injected Cameio CLI Console Logger\n\
      (function() {\n\
        ...
        function sendConsoleLog(method, args) {\n\
          try {\n\
            ...
            xhr.open("POST", "/__cameio-cli/console", true);\n\
...
function injectGoToScript(html) {
  try{
    ...
    return html.replace(insertAfter, insertAfter + '\n\
    <script>\n\
      // Injected Cameio Go To URL Live Reload Plugin\n\
      window.LiveReloadPlugin_CameioGoToUrl = (function() {\n\
        var GOTO_KEY = "__cameio_goto_url__";\n\
        var HISTORY_GO_KEY = "__cameio_history_go__";\n\
        var GoToUrlPlugin = function(window, host) {\n\
          this.window = window;\n\
          this.host = host;\n\
        }\n\
        GoToUrlPlugin.identifier = "__cameio_goto_url__";\n\
...
CameioTask.prototype._changed = function(filePath) {
...
CameioTask.prototype._goToUrl = function(url) {
  console.log( ('Loading: ' + url).green );
  this._postToLiveReload( ['__cameio_goto_url__' + url] );
};


CameioTask.prototype._goToHistory = function(goHistory) {
  goHistory = goHistory.replace('go(', '').replace(')', '');
  console.log( ('History Go: ' + goHistory).green );
  this._postToLiveReload( ['__cameio_history_go__' + goHistory] );
};


CameioTask.prototype._postToLiveReload = function(files) {
...
CameioTask.prototype.getAddress = function(cb) {
  try {
    var self = this;
    var addresses = [];
    var os = require('os');
    var ifaces = os.networkInterfaces();
    var cameioConfig = require('./config').load();

    var addressConfigKey = (self.isPlatformServe ? 'platformServeAddress' : 'cameioServeAddress');
    var savedAddress;

    if(self.isAddressCmd) {
      // reset any address configs
      cameioConfig.set('cameioServeAddress', null);
      cameioConfig.set('platformServeAddress', null);
    } else {
      savedAddress = cameioConfig.get(addressConfigKey);
    }
      ...
      prompt.get({properties: promptProperties}, function (err, promptResult) {
        ...
        for(var x=0; x<addresses.length; x++) {
          if(selection == (x + 1) || selection == addresses[x].address || selection == addresses[x].dev) {
            ...
            cameioConfig.set(addressConfigKey, self.address);
            cb();
            return;
          }
        }

        self.cameio.fail('Invalid address selection');
      });

    } else if(self.isPlatformServe) {
      // no addresses found
      self.cameio.fail('Unable to find an IPv4 address for run/emulate live reload.\nIs WiFi disabled or LAN disconnected?');
    ...
    self.cameio.fail('Error getting IPv4 address: ' + e);
  }
};

CameioTask.prototype.host = function(port) {
