var MixpanelAPI, crypto, Buffer, http, querystring, util, path, fs, os, CameioConfig;
...
var cameioConfig = CameioConfig.load();
...
exports.CameioStats = {
  t: function(additionalData) {
    try {
      ...
      if( cameioConfig.get('statsOptOut') === true ) {
        return;
      }
      ...
      // add which cameio lib version they're using
      try {
        statsData.cameio_version = require( path.resolve('www/lib/cameio/version.json') ).version;
      } catch(e2) {}
  ...
  mp: function(e, d) {
    var unique_id = cameioConfig.get('ank');
    if(!unique_id) {
      this.createId();
      unique_id = cameioConfig.get('ank');
    }
